...
import {
    fromJS, List as iList, Map as iMap,
}                             from 'immutable';
import { isEmpty, partial }   from 'lodash';
import {
    colors, lengths, ListItemTopBar, ObjectWidgetTopBar,
...
import {
    getErrorMessageForTypedFieldAndValue, getTypedFieldForValue,
    resolveFieldKeyType, TYPES_KEY,
...
const NestedObjectLabel = styled.div`
    display: ${props => (
    props.collapsed ? 'block' : 'none')};
    border-top: 0;
    color: ${props => (
    props.error ? colors.errorText : 'inherit')};
    background-color: ${colors.textFieldBorder};
    padding: 13px;
...
export default class OptionalObjectControl extends React.Component {
    ...
    handleAdd                = e => {
        ...
        const parsedValue                = this.getValueType() === valueTypes.SINGLE
            ? this.singleDefault() : fromJS(
                this.multipleDefault(field.get('fields')));
        this.setState({ itemsCollapsed: this.state.itemsCollapsed.push(false) });
        onChange((
                     value || iList()).push(parsedValue));
    };
    singleDefault            = () => {
    ...
    mixedDefault             = (typeKey, type) => {
        const selectedType = this.props.field.get(TYPES_KEY)
                                 .find(f => f.get('name') === type);
        const fields       = selectedType.get('fields') || [
            selectedType.get('field'),
        ];
    ...
    handleRemove             = (index, event) => {
        ...
        const collectionName                                         = field.get(
            'name');
        const isSingleField                                          = this.getValueType() === valueTypes.SINGLE;

        const metadataRemovePath = isSingleField ? value.get(index) : value.get(
            index)
                                                                           .valueSeq();
    ...
    handleCollapseAllToggle  = e => {
        ...
        this.setState({
                          itemsCollapsed: iList(Array(value.size)
                                                    .fill(!allItemsCollapsed)),
                      });
    };
    onSortEnd                = ({ oldIndex, newIndex }) => {
        ...
        const newValue = value.delete(oldIndex)
                              .insert(newIndex, item);
        this.props.onChange(newValue);

        // Update collapsing
        const collapsed             = itemsCollapsed.get(oldIndex);
        const updatedItemsCollapsed = itemsCollapsed.delete(oldIndex)
    ...
    renderItem               = (item, index) => {
        const {
                  classNameWrapper, editorControl, onValidateObject, metadata, clearFieldErrors, fieldsErrors, controlRef, resolveWidget,
              } = this.props;
        ...
        return (
            ...
            ]} index={index} key={`item-${index}`}>
                <StyledListItemTopBar collapsed={collapsed}
                                      onCollapseToggle={partial(
                                          this.handleItemCollapseToggle, index)}
                                      onRemove={partial(this.handleRemove,
                                                        index)}
                                      dragHandleHOC={SortableHandle}/>
                <NestedObjectLabel collapsed={collapsed}>{this.objectLabel(
                    item)}</NestedObjectLabel>
                <ClassNames>
                    {({ css, cx }) => (
                        <ObjectControl classNameWrapper={cx(classNameWrapper, {
                            [css`
    ...
    constructor (props) {
        ...
        const itemsCollapsed    = value && Array(value.size)
            .fill(allItemsCollapsed);
    ...
    handleChangeFor (index) {
        return (fieldName, newValue, newMetadata) => {
            ...
            const listFieldObjectWidget                = field.getIn(
                ['field', 'widget']) === 'object';
            const withNameKey                          = this.getValueType() !== valueTypes.SINGLE || (
                this.getValueType() === valueTypes.SINGLE && listFieldObjectWidget);
            const newObjectValue                       = withNameKey
                ? this.getObjectValue(index)
                      .set(fieldName, newValue) : newValue;
    ...
    renderErroneousTypedItem (index, item) {
        ...
        return (
            <SortableListItem css={[
                styles.listControlItem, styles.listControlItemCollapsed,
            ]} index={index} key={`item-${index}`}>
                <StyledListItemTopBar onCollapseToggle={null}
                                      onRemove={partial(this.handleRemove,
                                                        index)}
                                      dragHandleHOC={SortableHandle}/>
    ...
    renderListControl () {
        ...
        const labelSingular                             = field.get(
            'label_singular') || field.get('label', field.get('name'));
        const listLabel                                 = items.size === 1
            ? labelSingular.toLowerCase() : label.toLowerCase();
        ...
        return (
            <ClassNames>
                {({ cx, css }) => (
                    <div id={forID} className={cx(classNameWrapper, css`
                                ${styleStrings.objectWidgetTopBarContainer}
                            `)}>
                        <ObjectWidgetTopBar allowAdd={allowAdd}
                                            onAdd={this.handleAdd}
                                            types={field.get(TYPES_KEY, null)}
                                            onAddType={type => this.handleAddType(
                                                type,
                                                resolveFieldKeyType(field))}
                                            heading={`${items.size} ${listLabel}`}
                                            label={labelSingular.toLowerCase()}
                                            onCollapseToggle={this.handleCollapseAllToggle}
                                            collapsed={itemsCollapsed.every(
                                                val => val === true)}/>
