        ...
        return (
            <li className={`${childrenClass} ${openClass}`}>
                <Link to={link} activeStyle={{ color: '#ffb800' }}>
                    {linkText}
                    {hasChildren && <button type='button' onClick={toggleMenu.bind(this)}>{isOpen && <FontAwesomeIcon icon={faMinus} fixedWidth /> || <FontAwesomeIcon icon={faPlus} fixedWidth />}</button>}
                </Link>
                {!!hasChildren && (
                    <section>
                        <h1>{linkText}</h1>
